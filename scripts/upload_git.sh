...
if [[ -z "$GITHUB_TOKEN" ]]; then
  echo >&2 "Set the GITHUB_TOKEN env variable."
  exit 1
fi

# Ensure that there is a pattern specified.
if [[ -z "$1" ]]; then
  echo >&2 "Missing file (pattern) to upload."
  exit 1
fi
...
"*****************************************************************"
" "
" Artifacts are missing, and this action no longer invokes the "
" legacy-build script."
" "
" Please see the README.md file for github-action-publish-binaries"
" which demonstrates how to build AND upload artifacts."
" "
"*****************************************************************"
MISSING
    exit 1
